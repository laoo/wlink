//! Flash loader program for WCH's RISC-V MCUs.

/// For both CH32V20X and CH32V30X
// 5, 6
// flash op packet size = 128
pub const CH32V307: [u8; 446] = [
    ,
];

/// For CH32V205/CH32V203 (QingKe V3B new generation), a variant of the CH32V307 loader.
/// Captured from MounRiver Studio <-> WCH-LinkE (fw 2.22) traffic, sent as 512 bytes
/// (code padded with 0xff), firmware data follows in 4096-byte packs padded to 256 bytes.
// 0xce
pub const CH32V205: [u8; 512] = [
    ,
];

/// For CH32V103
// 1
pub const CH32V103: [u8; 494] = [
    ,
];

/// For CH56X
// 3
pub const CH569: [u8; 1156] = [
    ,
];

// 2
pub const CH573: [u8; 1102] = [
    ,
];

// 7, 11
pub const CH583: [u8; 1326] = [
    ,
];

/// For CH32V003.
// 9
pub const CH32V003: [u8; 498] = [
    ,
];
pub const CH32V003_ALT: [u8; 466] = [
    ,
];

// 10
pub const OP8571: [u8; 0x580] = [
    ,
];
// utility v2.2
pub const OP8571_ALT: [u8; 1386] = [
    ,
];

// CH643 and CH32X035
// 12
pub const CH643: [u8; 488] = [
    ,
];

// 14
pub const CH32L103: [u8; 0x200] = [
    ,
];

/// CH32V002/4/5/6/7, CH32M007
pub const CH32V00X: [u8; 500] = [
    ,
];

/// CH645
pub const CH645: [u8; 440] = [
    ,
];
pub const CH645_ALT: [u8; 486] = [
    ,
];

/// CH32V317
// 0x86
pub const CH32V317: [u8; 440] = [
    ,
];

pub const CH32V317_ALT: [u8; 460] = [
    ,
];

pub const CH32H417: [u8; 618] = [
    ,
];

// CH564
// 0xF
pub const CH564: [u8; 1532] = [
    ,
];
